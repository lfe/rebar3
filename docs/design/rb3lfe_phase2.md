...
2. rb3lfe_dependency_scanner:scan_file/2 → parses each file
...
7. rb3lfe_compiler_mod:needed_files/4 → uses DAG to find stale files
...
**File: `src/rb3lfe_dependency_scanner.erl`**

```erlang
-module(rb3lfe_dependency_scanner).
    ...
    case file:read_file(SourceFile) of
        {ok, Binary} ->
            ...
            IncludeDirs = maps:get(include_dirs, Opts,
                                   rb3lfe_config:get_include_dirs(AppInfo)),
...
**File: `src/rb3lfe_dep_cache.erl`**

```erlang
-module(rb3lfe_dep_cache).
...
%% ETS table name
-define(CACHE_TABLE, rb3lfe_dep_cache).
...
**File: `src/rb3lfe_dependency_scanner.erl` (UPDATE)**
...
scan_file(SourceFile, AppInfo, Opts) ->
    ...
    %% Check cache first
    case UseCache andalso rb3lfe_dep_cache:get(SourceFile) of
        {ok, CachedDeps, _Time} ->
        ...
        _ ->
            ...
            case file:read_file(SourceFile) of
                {ok, Binary} ->
                    ...
                    %% Cache the result
                    UseCache andalso rb3lfe_dep_cache:put(SourceFile, Result),
...
**File: `test/rb3lfe_dependency_scanner_SUITE.erl`**
...
# Run specific suite
rebar3 ct --suite=test/rb3lfe_dependency_scanner_SUITE
