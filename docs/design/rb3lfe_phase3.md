...
1. rb3lfe_compiler_mod:compile/4 → called by rebar3 for each file
2. rb3lfe_compile_worker:compile_file/3 → actual compilation
3. lfe_comp:file(Source, Opts) → LFE compiler
4. rb3lfe_compile_worker:format_result/2 → format output
5. Return {ok, Warnings} or {error, Errors, Warnings}
6. rb3lfe_compiler_mod:format_error/1 → human-readable errors
```
...
**File: `src/rb3lfe_compile_worker.erl`**

```erlang
-module(rb3lfe_compile_worker).
...
**File: `src/rb3lfe_compile_opts.erl`**

```erlang
-module(rb3lfe_compile_opts).
...
-define(OPTS_CACHE, rb3lfe_opts_cache).
        ...
        TargetTime ->
            ...
            if
                ...
                true ->
                    ...
                    OptsChanged = rb3lfe_compile_opts:opts_changed(Source, Opts),
...
**File: `src/rb3lfe_progress.erl`**

```erlang
-module(rb3lfe_progress).
...
**File: `test/rb3lfe_compile_worker_SUITE.erl`**
...
compile_with_includes(Config) ->
    ...
    Opts = [{i, IncludeDir}],
    Result = rb3lfe_compile_worker:compile_file(SourceFile, OutDir, Opts),
...
option_change_triggers_recompile(Config) ->
    ...
    Opts1 = [verbose],
    ok = rb3lfe_compile_worker:compile_file(SourceFile, EbinDir, Opts1),
    rb3lfe_compile_opts:save_opts_hash(SourceFile, Opts1),

    %% Check if different options trigger recompile
    Opts2 = [verbose, debug_info],
    OptsChanged = rb3lfe_compile_opts:opts_changed(SourceFile, Opts2),
...
first_files_compiled_first(Config) ->
    ...
    %% Get context
    Context = rb3lfe_compiler_mod:context(AppInfo1),
...
parallel_compilation_support(Config) ->
    ...
    %% Call needed_files
    Result = rb3lfe_compiler_mod:needed_files(G, Files, OutMappings, AppInfo),
...
# Run specific suite
rebar3 ct --suite=test/rb3lfe_compile_worker_SUITE
