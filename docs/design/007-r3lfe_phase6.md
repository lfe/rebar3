...
**File: `test/r3lfe_properties.erl`**
...
**File: `test/r3lfe_benchmarks.erl`**

```erlang
-module(r3lfe_benchmarks).
...
run_all() ->
    io:format("~n=== r3lfe Performance Benchmarks ===~n~n"),
...
release_workflow(Config) ->
    ...
    %% Compile
    State = create_rebar_state(TestDir),
    {ok, State1} = r3lfe_prv_compile:do(State),

    %% Build release
    {ok, _} = r3lfe_prv_release:do(State1),
  ...
  benchmarks:
    ...
    steps:
      ...
      - name: Run benchmarks
        run: |
          rebar3 compile
          rebar3 shell --eval "r3lfe_benchmarks:run_all()"
...
PROJECT = r3lfe
ROOT_DIR = $(shell pwd)
SYS_TEST_DIR = /tmp/r3lfe/_integration/_testing
...
- Plugin renamed from `rebar3_lfe` to `r3lfe`
- All internal modules renamed with `r3lfe_` prefix
