...
**Goal**: Complete, cohesive documentation covering all r3lfe features.
...
r3lfe includes several project templates that are automatically available when the plugin is loaded. These templates follow rebar3's template system and are stored in `priv/templates/`.
...
Templates are maintained in the r3lfe repository:
...
**Quick Migration:**
1. Update `rebar.config`: `{plugins, [{r3lfe, "0.5.0"}]}`
...
2. **Version Update:**
   ```bash
   vim src/r3lfe.app.src  # Set version to 0.5.0
...
The complete rewrite of rebar3_lfe as r3lfe is done!
