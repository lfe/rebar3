-module(r3lfe_prv_defabulate_SUITE).
...
%% Test cases
-export([
    defabulate_provider_registers/1,
    defabulate_simple_data/1,
    defabulate_complex_data/1,
    defabulate_with_output_option/1,
    defabulate_force_overwrite/1,
    defabulate_no_input_error/1,
    defabulate_input_not_found_error/1,
    defabulate_output_exists_error/1,
    defabulate_multiple_forms/1,
    defabulate_nested_structures/1,
    defabulate_format_error_variants/1,
    defabulate_parse_error/1,
    defabulate_single_form_list/1,
    defabulate_single_form_nonlist/1,
    defabulate_info_output/1
]).
...
all() ->
    [
        defabulate_provider_registers,
        defabulate_simple_data,
        defabulate_complex_data,
        defabulate_with_output_option,
        defabulate_force_overwrite,
        defabulate_no_input_error,
        defabulate_input_not_found_error,
        defabulate_output_exists_error,
        defabulate_multiple_forms,
        defabulate_nested_structures,
        defabulate_format_error_variants,
        defabulate_parse_error,
        defabulate_single_form_list,
        defabulate_single_form_nonlist,
        defabulate_info_output
    ].
...
init_per_testcase(_TestCase, Config) ->
    TestDir = test_utils:create_temp_dir("defabulate"),
...
defabulate_provider_registers(_Config) ->
    State = rebar_state:new(),

    {ok, State1} = r3lfe_prv_defabulate:init(State),
...
defabulate_complex_data(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_with_output_option(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_force_overwrite(Config) ->
    TestDir = ?config(test_dir, Config),
    ...
    Result1 = r3lfe_prv_defabulate:do(State1),
    ...
    Result2 = r3lfe_prv_defabulate:do(State2),
...
defabulate_no_input_error(_Config) ->
    State = rebar_state:new(),
    State1 = rebar_state:command_parsed_args(State, {[], []}),
...
defabulate_input_not_found_error(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_output_exists_error(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_multiple_forms(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_nested_structures(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_format_error_variants(_Config) ->
    ...
    lists:foreach(
        fun(Error) ->
            Result = r3lfe_prv_defabulate:format_error(Error),
...
defabulate_single_form_list(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_single_form_nonlist(Config) ->
    TestDir = ?config(test_dir, Config),
...
defabulate_info_output(_Config) ->
    %% Test that info/1 returns proper documentation
    Info = r3lfe_prv_defabulate:info("Test Description"),
