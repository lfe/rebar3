...
templates_directory_exists(_Config) ->
    %% Get priv dir - may be rebar3_lfe or rb3lfe depending on load order
    PrivDir = case code:priv_dir(rebar3_lfe) of
        {error, bad_name} ->
            %% Try alternate name
            code:priv_dir(rb3lfe);
        Dir ->
            Dir
    end,
    ...
    case PrivDir of
        {error, bad_name} ->
            ct:pal("Warning: Neither rebar3_lfe nor rb3lfe application loaded, skipping template tests");
        _ ->
